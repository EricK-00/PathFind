...
public static partial class GFunc
{
    ...
    {
        T resultComponent = default;
        GameObject childObj = default;

        childObj = targetObj_.FindChildObj(objName_);
        if (childObj.IsValid())
        {
            resultComponent = childObj.GetComponentMust<T>();
        }
    ...
    public static List<GameObject> GetChildrenObjs(this GameObject targetObj_)
    {
        ...
        {
            searchTarget = targetObj_.transform.GetChild(i).gameObject;
            resultObjs.Add(searchTarget);
        }

        if (resultObjs.IsValid())
            return resultObjs;
        else
